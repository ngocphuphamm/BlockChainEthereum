...
describe("PNP",async ()=>{
    ...
    describe("unpause()",async ()=>{
        ...
        it("should pause contract correctly",async ()=>{
            ...
            let transferTx = await contract.transfer(accountB.adress,amount);
            await expect(transferTx).to.emit(contract,'Transfer').withArgs(accountA.address,accountB.adress);

        })
    })
    describe("addToBlackList()",()=>{
        it("should revert in case add sender to blacklist", async ()=>{
            await expect(contract.addToBlackList(accountB.address)).to.be.revertedWith("PNP: must not add sender to black list");
        })
        it("should revert if account has been added to blacklist", async ()=>{
            await contract.addToBlackList(accountA.address);
            await expect(contract.addToBlackList(accountB.address)).to.be.rejectedWith("PNP : ACCOUNT WAS ON BLACKLIST");
        })
    ...
    describe("removeFromBlackList()",() =>{ 
        ...
        it("should revert if account has not been added to blacklist", async ()=>{
            await contract.removeFromBlackList(accountB.address);
            await expect(contract.removeFromBlackList(accountB.address)).to.be.rejectedWith("PNP : ACCOUNT WAS not ON BLACKLIST");
        })
        it("should revert if not admin role ", async () =>{
            await expect(contract.connnect(accountB.address).removeFromBlackList(accountC.address)).to.be.reverted;
        })
